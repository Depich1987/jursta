...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO commune (id_commune, id_departement, id_admin, date_creation) VALUES (%s, %s, %s, %s)",
...
mysql_select_db($database_jursta, $jursta);
$query_Liste_id_departement = "SELECT * FROM departement ORDER BY id_departement ASC";
$Liste_id_departement = mysql_query($query_Liste_id_departement, $jursta) or die(mysql_error());
$row_Liste_id_departement = mysql_fetch_assoc($Liste_id_departement);
$totalRows_Liste_id_departement = mysql_num_rows($Liste_id_departement);
?>
<HTML>
<HEAD>
        ...
        <tr>
          <td valign="middle"><table width="100%" border="0" cellpadding="5" cellspacing="0">
              <tr>
                ...
                <td><input name="id_commune" type="text" id="id_commune" size="30"></td>
              </tr>
              <tr>
                <td align="right" valign="top" nowrap class="Style10">Departement : </td>
                <td><select name="id_departement" id="id_departement">
                  <?php
do {  
?>
                  <option value="<?php echo $row_Liste_id_departement['id_departement']?>" selected><?php echo $row_Liste_id_departement['lib_departement']?></option>
                  <?php
} while ($row_Liste_id_departement = mysql_fetch_assoc($Liste_id_departement));
  $rows = mysql_num_rows($Liste_id_departement);
  if($rows > 0) {
      mysql_data_seek($Liste_id_departement, 0);
	  $row_Liste_id_departement = mysql_fetch_assoc($Liste_id_departement);
  }
?>
                </select>
                  <input type="button" name="id_departement_cmd" id="id_departement_cmd" value="..." onClick="javascript:ouvre_popup('add_organisation_departement.php',495,180)"></td>
              </tr>
              <tr>
...
mysql_free_result($Liste_id_departement);
?>
