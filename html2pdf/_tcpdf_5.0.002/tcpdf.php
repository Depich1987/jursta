...
if (!class_exists('TCPDF', false)) {
	...
	*/
	class TCPDF {
		...
		/**
		 * Append a cubic B&eacute;zier curve to the current path. The curve shall extend from the current point to the point (x3, y3), using (x1, y1) and (x2, y2) as the B&eacute;zier control points.
		...
		/**
		 * Append a cubic B&eacute;zier curve to the current path. The curve shall extend from the current point to the point (x3, y3), using the current point and (x2, y2) as the B&eacute;zier control points.
		 * The new current point shall be (x3, y3).
		 * @param float $x2 Abscissa of control point 2.
		 * @param float $y2 Ordinate of control point 2.
		 * @param float $x3 Abscissa of end point.
		...
		/**
		 * Append a cubic B&eacute;zier curve to the current path. The curve shall extend from the current point to the point (x3, y3), using (x1, y1) and (x3, y3) as the B&eacute;zier control points.
		 * The new current point shall be (x3, y3).
		 * @param float $x1 Abscissa of control point 1.
		 * @param float $y1 Ordinate of control point 1.
		 * @param float $x2 Abscissa of control point 2.
		...
		protected function SVGPath($d, $style='') {
			...
			foreach ($paths as $key => $val) {
				...
				switch (strtoupper($cmd)) {
					...
					case 'Q': { // quadratic B&eacute;zier curveto
						foreach ($params as $ck => $cp) {
							$params[$ck] = $cp;
							if ((($ck + 1) % 4) == 0) {
					...
					case 'T': { // shorthand/smooth quadratic B&eacute;zier curveto
						foreach ($params as $ck => $cp) {
							$params[$ck] = $cp;
							if (($ck % 2) != 0) {
