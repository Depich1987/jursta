...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE reg_socialopposition SET noordre_socialopp=%s, datejour=%s, datejugdefaut=%s, datesignification=%s, newdataudience=%s, signature=%s, no_decision=%s, Id_admin=%s, date_creation=%s, date_modif=%s, id_juridiction=%s WHERE id_socialopp=%s",
                       ...
                       GetSQLValueString($_POST['id_socialopp'], "int"));
...
if (isset($_GET['idros'])) {
  $colname_select_regopp = $_GET['idros'];
}
mysql_select_db($database_jursta, $jursta);
$query_select_regopp = sprintf("SELECT * FROM reg_socialopposition, rg_social, rep_decision WHERE ((id_socialopp = %s) AND (rg_social.no_rgsocial=rep_decision.no_rgsocial) AND (rep_decision.no_decision=reg_socialopposition.no_decision))", GetSQLValueString($colname_select_regopp, "int"));
...
if (isset($_GET['idros'])) {
  $colname_select_date = $_GET['idros'];
}
mysql_select_db($database_jursta, $jursta);
$query_select_date = sprintf("SELECT date_creation, date_modif FROM reg_socialopposition WHERE id_socialopp = %s", GetSQLValueString($colname_select_date, "int"));
...
rechargerpage("liste_regoppositionsocial.php");
</script>
<?php
}
?>
        ...
        <tr>
          <td valign="middle"><table width="100%" border="0" cellpadding="5" cellspacing="0">
              <tr>
                ...
                <td bgcolor="#FFFF00" class="Style10"><?php echo $row_select_regopp['noordre_rgsocial']; ?></td>
                </tr>
              <tr>
                <td align="right" valign="top" nowrap class="Style10">Demandeur : </td>
                <td class="Style10">                  <?php echo $row_select_regopp['demandeur_rgsocial']; ?></td>
              </tr>
              <tr>
                <td align="right" valign="top" nowrap class="Style10"> Defendeur : </td>
                <td class="Style10"><?php echo $row_select_regopp['defendeur_rgsocial']; ?></td>
              </tr>
                </table></td>
              </tr>
              <tr>
                ...
                <td class="Style10"><label>
                  <input name="noordre_socialopp" type="text" id="noordre_socialopp" value="<?php echo $row_select_regopp['noordre_socialopp']; ?>" size="15">
                  </label></td>
              </tr>
              <tr>
              ...
              <tr>
                <td nowrap><input name="id_admin" type="hidden" id="id_admin" value="<?php echo $row_select_admin['Id_admin']; ?>">
                  <input name="date_creation" type="hidden" id="date_creation" value="<?php echo $row_select_regopp['date_creation']; ?>">
                  <input name="date_modif" type="hidden" id="date_modif" value="<?php echo date("Y-m-d H:i:s"); ?>">
                  <input name="id_socialopp" type="hidden" id="id_socialopp" value="<?php echo $row_select_regopp['id_socialopp']; ?>">
                  <input name="datejour" type="hidden" id="datejour" value="<?php echo $row_select_regopp['datejour']; ?>">
                  <input name="no_decision" type="hidden" id="no_decision" value="<?php echo $row_select_regopp['no_decision']; ?>">
