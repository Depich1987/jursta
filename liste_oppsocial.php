...
if ((isset($_GET['id_socialopp'])) && ($_GET['id_socialopp'] != "")) {
  $deleteSQL = sprintf("DELETE FROM reg_socialopposition WHERE id_socialopp=%s",
                       GetSQLValueString($_GET['id_socialopp'], "int"));
...
mysql_select_db($database_jursta, $jursta);
$query_liste_oppositionsocial = "SELECT * FROM reg_socialopposition, rep_decision, rg_social WHERE ((rg_social.no_rgsocial=rep_decision.no_rgsocial)AND(reg_socialopposition.no_decision=rep_decision.no_decision) AND ((reg_socialopposition.noordre_socialopp LIKE '".$_GET['wrdkey']."%') OR (rg_social.demandeur_rgsocial LIKE '".$_GET['wrdkey']."%') OR (rg_social.defendeur_rgsocial LIKE '".$_GET['wrdkey']."%'))) ORDER BY id_socialopp DESC";
                                            ...
                                            <td valign="middle"><table >
                                                <tr align="center" class="Style16">
                                                  ...
                                                  <?php if ($row_select_admin['supprimer_admin']==1) { ?>
                                                    <a href="liste_oppositionsocial.php?idros=<?php echo $row_liste_oppositionsocial['id_socialopp']; ?>" onClick="return confirmdelete('Voulez-vous supprimer ?')"><img src="images/cancel_f2.png" title="Supprimer" width="28" border="0"></a>
                                                    <?php } else {?>
