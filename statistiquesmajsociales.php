...
$sql_insert_anneesociale="";
$sql_insert_t30sociale="";
$sql_insert_maj_t30sociale="";
$delete_t36_majsociale="";
$sql_insert_maj_t36sociale="";
$maj_insertSQL_finalsociale="";
...
for ($annee=2011;$annee<=substr(date('Y-m-d'),0,4);$annee++){
	...
	if ($totalRows_select_annee==0){
		...
		$sql_insert_anneesociale=$insertSQL;		
	}
	else{
		$sql_insert_anneesociale="";
	}
...
if ($totalRows_select_liaison==0){
	$sql_insert_t30sociale="SELECT @nbenrgsociale=MAX(T30_NUMERO) FROM t30_fs;";
	$insertSQL = "INSERT INTO t30_fs (T06_ANNEE, T30_MOIS, id_juridiction,T30_TRIM,T30_SEM) VALUES (".$annee.",".$mois.",".$juridiction.",".$trimestre.",".$semestre.");";
  	mysql_select_db($database_jursta, $jursta);
  	$Result1 = mysql_query($insertSQL, $jursta) or die(mysql_error());
	$sql_insert_maj_t30sociale=$insertSQL.$sql_insert_t30sociale;		
	$numero_liaison=mysql_insert_id();
}
else{
	$sql_insert_maj_t30sociale="SELECT @nbenrgsociale=T30_NUMERO FROM t30_fs WHERE ((t30_fs.T06_ANNEE=".$annee.")"." AND (t30_fs.T30_MOIS=".$mois.") AND (id_juridiction=".$juridiction."));";
...
$insertSQL = sprintf("INSERT INTO T36_DFSCC (T36_NBENROLES, T36_JR_CONTRA, T36_JR_DEF, T36_JR_ACCDEM, T36_JR_REJET, T36_JR_INCOMP, T36_JR_RADIATION, T36_DUREEMOY, T36_RAJ, T34_CODE, T36_NBACT, T36_NBAFFPREC,T30_NUMERO) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);",
  ...
  $string_replace="@nbenrgsociale);";
  $sql_insert_maj_t36sociale=str_replace($string_search,$string_replace,$insertSQL);
  $maj_insertSQL_finalsociale=$sql_insert_anneesociale.$maj_insertSQL_finalsociale.$sql_insert_maj_t30sociale.$sql_insert_maj_t36sociale;
?> 
<?php 	}
	}
	?>
...
$delete_t36_majsociale=$delete_t36_majsociale."SELECT @nbenrgsocialesup=T30_NUMERO FROM t30_fs WHERE (id_juridiction=".$juridiction.");DELETE FROM T36_DFSCC WHERE (T30_NUMERO=@nbenrgsocialesup);";
?>
<?php } while ($row_liste_t30 = mysql_fetch_assoc($liste_t30)); ?>
<?php } ?>

<?php $maj_insertSQL_final_total.="DECLARE @nbenrgsociale bigint; DECLARE @nbenrgsocialesup bigint;".$delete_t36_majsociale.$maj_insertSQL_finalsociale; ?>
